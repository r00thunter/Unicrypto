...
class Requests{
	...
	public static function insertDeposit($currency=false,$amount=false,$account_number=false,$transaction_id=false){
		global $CFG;
		...
		$status = $CFG->request_pending_id;
			$request_id = db_insert('requests',array('date'=>date('Y-m-d H:i:s'),'site_user'=>User::$info['id'],'currency'=>$currency,'amount'=>$amount,'description'=>$CFG->deposit_fiat_desc,'request_status'=>$status,'request_type'=>$CFG->request_deposit_id,'account'=>$account_number,'net_amount'=>$amount , 'transaction_id' =>$transaction_id ));
			db_insert('history',array('date'=>date('Y-m-d H:i:s'),'ip'=>$CFG->client_ip,'history_action'=>$CFG->history_deposit_id,'site_user'=>User::$info['id'],'request_id'=>$request_id,'balance_before'=>User::$info[strtolower($currency_info['currency'])],'balance_after'=>(User::$info[strtolower($currency_info['currency'])] - $amount)));
			
			if (User::$info['notify_deposit_bank'] == 'Y') {
			    $info['amount'] = number_format($amount,2);
			    $info['currency'] = $currency_info['currency'];
			    $info['first_name'] = User::$info['first_name'];
			    $info['last_name'] = User::$info['last_name'];
	...
	// Private API
	public static function get_private_api($count=false,$page=false,$per_page=false,$withdrawals=false,$currency=false,$status=false,$public_api=false,$id=false,$session_user=false) {
		global $CFG;
	...
	public static function insert_private_api($currency=false,$amount=false,$btc_address=false,$account_number=false,$session_user=false) {
		global $CFG;
		...
		$currency_info = $CFG->currencies[$currency];
		$available = User::getAvailable_private_api($session_user);
		$is_crypto = ($currency_info['is_crypto'] == 'Y');
		$amount = round($amount,($is_crypto ? 8 : 2),PHP_ROUND_HALF_UP);
		if ($amount > $available[$currency_info['currency']])
			return false;
		...
		if ($request_id > 0) {
			$result = self::get_private_api(false,false,false,false,false,false,1,$request_id,$session_user);
			return $result[0];
		}
		else
			return $request_id;
	...
	//  get user balance
	public static function getUserBalance($currency=false) {

			if ($currency) {
				$user_id = User::$info['id'];
				$site_users_balances = "SELECT balance FROM site_users_balances WHERE site_user = $user_id AND currency = $currency";
				$result = db_query_array($site_users_balances);
				return $result;
			}
			return false;
	...
	//  get user balance tans
	public static function getUserBalanceTrans($currency=false) {

			if ($currency) {
				$user_id = User::$info['id'];
				$site_users_balances = "SELECT balance FROM site_users_balances_trans WHERE site_user = $user_id AND currency = $currency";
				$result = db_query_array($site_users_balances);
				return $result;
			}
			return false;
	...
	//  store user balance tans
	public static function insertUserBalanceTrans($currency=false,$available_balance=false) {

			if ($currency) {
				$user_id = User::$info['id'];
				$site_users_balances = db_insert('site_users_balances_trans',array('balance'=>$available_balance,'site_user'=>User::$info['id'],'currency'=>$currency));
				return $result;
			}
			return false;
	...
	//  afteruser balance tans check update user balance trans
	public static function updatetUserBalanceTrans($currency=false,$calculate_user_balance=false) {

			if ($currency && $calculate_user_balance) {
				$user_id = User::$info['id'];
				$update_site_users_balances = "UPDATE `site_users_balances_trans` SET `balance`= $calculate_user_balance WHERE `site_user` = $user_id AND `currency` = $currency";
				$result = db_query($update_site_users_balances);
				return $result;
			}
			return false;
	...
	//  afteruser balance tans check update user balance
	public static function updatetUserBalance($currency=false,$calculate_user_balance=false) {

			if ($currency && $calculate_user_balance) {
				$user_id = User::$info['id'];
				$update_site_users_balances = "UPDATE `site_users_balances` SET `balance`= $calculate_user_balance WHERE `site_user` = $user_id AND `currency` = $currency";
				$result = db_query($update_site_users_balances);
				return $result;
			}
			return false;
	...
	//  Received Transaction Id check
	public static function receivedTransactionIdCheck($transaction_id) {

			// if ($transaction_id) {
				// $user_id = User::$info['id'];
				$sql = 'SELECT transaction_id FROM requests WHERE transaction_id = "'.$transaction_id.'"';
				$result = db_query_array($sql);
				if ($result) {
					 return true;
				}				
	...
	//  Insert Received Transaction Details
	public static function receivedTransactionInsert($user_id,$currency=false,$date=false,$amount=false,$btc_address=false,$hash=false) {

			if ($currency && $amount) {
				$user_id = User::$info['id'];
				$request_id = db_insert('requests',array('date'=>$date,'site_user'=>$user_id,'currency'=>$currency,'amount'=>$amount,'description'=>'4','request_status'=>'2','request_type'=>'2','send_address'=>$btc_address, 'transaction_id'=>$hash, 'fee'=>'0','net_amount'=>$amount));
				return $result;
			}
			return false;
	...
	//  Insert Received Transaction Details
	public static function sendTransactionInsert($user_id,$currency=false,$date=false,$amount=false,$btc_address=false,$hash=false) {

			if ($currency && $amount) {
				$user_id = User::$info['id'];
				$request_id = db_insert('requests',array('date'=>$date,'site_user'=>$user_id,'currency'=>$currency,'amount'=>$amount,'description'=>'2','request_status'=>'2','request_type'=>'1','send_address'=>$btc_address, 'transaction_id'=>$hash, 'fee'=>'0','net_amount'=>$amount));
				return $result;
			}
			return false;
	...
	//  after withdraw success reduce user balance
	public static function reduceUserBalance($currency=false,$calculate_user_balance=false) {
	...
	//  after deposit api success add user balance
	public static function addUserBalance($currency=false,$calculate_user_balance=false,$status=false) {
			...
			if ($currency && $calculate_user_balance) {
				...
				$update_site_users_balances = db_insert('site_users_balances',array('balance'=>$calculate_user_balance,'site_user'=>User::$info['id'],'currency'=>$currency));
				}
				$result = db_query($update_site_users_balances);
				return $result;
			}

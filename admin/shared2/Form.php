...
class Form {
			...
			if (!$is_tokenizer || strstr($value,'array:')) {
				if (strstr($value,'array:')) {
					...
					elseif (strlen($value1) > 0) {
						$tokenizer_values = Stringz::unFaux($value1);
					}
				}
				else {
	...
	function button($url=false,$value=false,$variables=false,$target_elem_id=false,$id=false,$class = false, $jscript = false, $style = false,$disable_if_no_record_id=false,$disable_if_cant_edit=false,$static=false,$update_variable_values=false,$bypass_create_record=false) {
		...
		$variables1 = Stringz::parseVariables($variables,$this->info,$this->record_id,$url,$update_variable_values);
	...
	function link($url=false,$caption=false,$variables=false,$target_elem_id=false,$class=false,$disable_if_no_record_id=false,$disable_if_cant_edit=false) {
		...
		$variables1 = Stringz::parseVariables($variables,$this->info,$this->record_id,$url);
		$variables1['bypass_save'] = 1;
		$permission_level = ($variables1['action'] == 'form') ? 2 : 1;
		$disabled = (User::permission(0,0,$url,false,$variables1['is_tab']) <= $permission_level);
		$disabled = (!($this->record_id > 0) && $disable_if_no_record_id) ? true : $disabled;
	...
	function heading($caption) {
		...
		$variables1 = Stringz::parseVariables($variables[1],$this->info,$this->record_id);
	...
	function startArea($legend,$class=false,$height=false) {
		...
		$matches = Stringz::getSubstring($legend,'[',']');
		if (is_array($matches)) {
	...
	function startRestricted($groups=false,$only_admin=false,$users=false,$user_id_equals_field=false,$group_id_equals_field=false,$condition=false,$exclude_groups=false,$exclude_admin=false,$exclude_users=false) {
		...
		if ($CFG->pm_editor) {
			...
			if ($only_admin) {
				if ($condition) {
					$condition = Stringz::doFormulaReplacements($condition,$this->info,1,1);
						...
						case true:
							...
							elseif (!$CFG->backstage_mode) {
								if ($_REQUEST['db_fields'][$name] == 'vchar' || $_REQUEST['db_fields'][$name] == 'int') {
									...
									if (Stringz::sanitize($value,true,$delete_whitespace)) {
		...
		if ($this->info && !$this->errors) {
			...
			if (!$this->errors) {
				...
				if (!$this->record_id) {
					...
					$lastRecord = DB::getRecord($this->table,$this->record_id) ;
					if (DB::update($this->table,$insert_values,$this->record_id,$this->ignore_fields) != -1) {
						
						if($this->table == "requests"){

							if($insert_values['currency'] == 27 && $insert_values['request_status'] == 2 && $insert_values['request_status'] != $lastRecord['request_status']){
								$sql = "SELECT site_users_balances.* FROM site_users_balances WHERE site_user = '".$insert_values['site_user']."' AND currency = ".$insert_values['currency']." LIMIT 0,1" ;
								$userBalanceResult =  db_query_array($sql) ;
								if(!empty($userBalanceResult)){
									$userBalance = $userBalanceResult[0] ;
									if($insert_values['request_type'] == 2)
										$userBalance['balance'] = $userBalance['balance']+$insert_values['amount'] ;
									else if($insert_values['request_type'] == 1){
										$userBalance['balance'] = $userBalance['balance']-$insert_values['amount'] ;
										if($userBalance['balance'] < 0){
											$this->errors[$this->record_id] = "User doesn't have enough balance for this Withdrawal" ;
										}
									}
									if(!$this->errors[$this->record_id]){
										DB::update("site_users_balances",$userBalance, $userBalance['id']) ;
										
										$wuser = "SELECT * FROM site_users WHERE id = '".$insert_values['site_user']."'";
										$currency = "SELECT * FROM currencies WHERE id = '".$insert_values['currency']."'";
										$wusers =  db_query_array($wuser) ;
										$currencys =  db_query_array($currency) ;
										$vars = $wusers;
										$vars['exchange_name'] = 'Bitexchange';
										$vars['amount'] = $insert_values['amount'];
										$vars['currency'] = $currencys[0]['currency'];
										$email = SiteEmail::getRecord('withdraw-completed');
										Email::send($CFG->form_email,$wusers[0]['email'],$email['title_en'],$CFG->form_email_from,false,$email['content_en'],$vars);
									}
								}else{
									if($insert_values['request_type'] == 2){
										$userBalance = array() ;
										$userBalance['site_user'] = $insert_values['site_user'] ;
										$userBalance['currency'] = $insert_values['currency'] ;
										$userBalance['balance'] = $insert_values['net_amount'] ;
										db_insert("site_users_balances",$userBalance) ;
									}else{
										$this->errors[$this->record_id] = "User doesn't have enough balance for this Withdrawal" ;										
									}
								}
...
*/
		...
		if ($send_condition) {
			$send_condition = Stringz::doFormulaReplacements($send_condition,$this->info,1);
...
*/
				...
				if (is_array($insert_array)) {
					foreach ($insert_array as $new_field =>$old_field) {
						...
						elseif (strstr($old_field,'(') && strstr($old_field,')')) {
							...
							$formula = Stringz::doFormulaReplacements($old_field,$this->old_info,1);
	...
	private function parseJscript($jscript,$id,$j=false,$grid_input=false) {
		...
		$matches = Stringz::getSubstring($jscript,'[',']');
		if (is_array($matches)) {
	...
	function filterMonth($field_name,$caption=false,$language=false) {
		...
		self::selectInput($field_name.'_month',$caption,false,false,Stringz::getMonthNames($language));
		$CFG->o_method_suppress = true;

...
class Comments {
	...
	function get($p_id=0,$count=false) {
		...
		if ($sql_filter) {
			$matches = Stringz::getSubstring($sql_filter,'[',']');
		...
		if ($sql_filter) {
			$sql_filter = Stringz::doFormulaReplacements($sql_filter);
			$sql .= " AND (".$sql_filter.') ';
		}
	...
	private function show($comments) {
		...
		if ($comments) {
			...
			foreach ($comments as $comment) {
				...
				$action = Stringz::doFormulaReplacements($action,unserialize($comment['f_table_row']),1,1);

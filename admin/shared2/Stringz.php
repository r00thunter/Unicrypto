...
class Stringz {
	...
	public static function fauxArray($array,$not_root=false) {
		...
		foreach ($array as $k=>$v) {
			if (@strlen($v) > 0 || is_array($v)) {
				if (is_array($v)) {
					$ret[] = $k.'=>('.Stringz::fauxArray($v,true).')';
				}
				else {
					$ret[] = $k.'=>'.$v;
				}
	...
	public static function magicQuotesOff() {
		if (get_magic_quotes_gpc()) {
  			$_GET = Stringz::stripQuotes($_GET);
  			$_POST = Stringz::stripQuotes($_POST);
  			$_REQUEST = Stringz::stripQuotes($_REQUEST);
  			$_COOKIES = Stringz::stripQuotes($_COOKIES);
		}
	}
	...
	public static function truncate($text, $length = 100, $ending = '...', $exact = false, $considerHtml = true) {
		...
		if ($considerHtml) {
			...
			foreach ($lines as $line_matchings) {
				...
				if ($total_length+$content_length> $length) {
					...
					Stringz::$missing = substr($line_matchings[2],$left+$entities_length);
		...
		if (!$exact) {
			...
			if (isset($spacepos)) {
				...
				Stringz::$missing = substr($truncate,$spacepos).Stringz::$missing;
	...
	function doFormulaReplacements($formula,$row=false,$directly_link_fields=false,$use_provided_row=false) {
		...
		$matches = Stringz::getSubstring($formula,'[',']');
	...
	public static function replaceConditionals($formula,$row,$f_id_field=false) {
		...
		if (is_array($conditionals[2])) {
			...
			foreach ($conditionals[2] as $conditional) {
				$matches = Stringz::getSubstring($return_values[2][$i],'[',']');
				$matches = (count($matches) > 0) ? $matches : array();
				$matches1 = Stringz::getSubstring($conditional,'[',']');
				...
				$matches = Stringz::getSubstring($conditional,'[',']');
				if ($matches) {
					foreach ($matches as $match) {
						if (strstr($match,',')) {
							$match_parts = explode(',',$match);
		...
		if (is_array($variables)) {
			foreach ($variables as $k => $v) {
				...
				if (strstr($v1,'(') && strstr($v1,')')) {
					$formula = Stringz::doFormulaReplacements($v1,$row,1);

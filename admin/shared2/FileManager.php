			...
			if ($this->data) {
				foreach ($this->data as $file) {
					if ($file['alert_condition1']) {
						...
						$condition = Stringz::doFormulaReplacements($file['alert_condition1'],$file_row,1);
					...
					if ($file['alert_condition2']) {
						...
						$condition = Stringz::doFormulaReplacements($file['alert_condition2'],$file_row,1);
				...
				foreach ($all_data as $row) {
					if (array_key_exists('file_name',$row)) {
						if ($row['alert_condition1']) {
							...
							$condition = Stringz::doFormulaReplacements($row['alert_condition1'],$file_row,1);
							$alert_class1 = (eval("if ($condition) { return 1;} else { return 0;}")) ? 'alert1' : '';
						}
						if ($row['alert_condition2']) {
							$file_row = (is_array($file_row)) ? $file_row : DB::getRecord($row['table'],$row['id']);
							$condition = Stringz::doFormulaReplacements($row['alert_condition2'],$file_row,1);
							$alert_class2 = (eval("if ($condition) { return 1;} else { return 0;}")) ? 'alert2' : '';
						}
			...
			foreach ($this->filters as $filter) {
				...
				switch ($filter['type']) {
					...
					case 'month':
						...
						$form_filters->selectInput($name.'_month',$filter['caption'],false,false,Stringz::getMonthNames($filter['language']));
						$CFG->o_method_suppress = true;

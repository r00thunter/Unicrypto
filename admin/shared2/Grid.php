...
class Grid {
	...
	function aggregate($name,$formula,$header_caption=false,$cumulative_function=false,$run_in_sql=false,$filter=false,$join_path=false,$join_condition=false) {
		...
		$this->fields[$name] = array(
			'name'=>$name,
			'formula'=>$formula,
			'header_caption'=>($header_caption) ? $header_caption : Stringz::substring($formula,15),
			'method_id'=>$CFG->method_id,
			...
			if (is_array($data)) {
				...
				foreach ($data as $row) {
					...
					if ($this->alert_condition1) {
						$condition = Stringz::doFormulaReplacements($this->alert_condition1,$row,1);
						$alert_class1 = (${("if ($condition) { return 1;} else { return 0;}")}) ? 'alert1' : '';
					}
					if ($this->alert_condition2) {
						$condition = Stringz::doFormulaReplacements($this->alert_condition2,$row,1);
						$alert_class2 = (${("if ($condition) { return 1;} else { return 0;}")}) ? 'alert2' : '';
					}
					...
					foreach ($this->fields as $name => $properties) {
						...
						elseif ($properties['is_form']) {
							...
							if (!empty($properties['insert_new_record_when'])) {
								$properties['insert_new_record_when'] = Stringz::replaceConditionals('('.$properties['insert_new_record_when'].')',$row,$properties['f_id_field']);
								$result = ${("if ({$properties['insert_new_record_when']}) { return 0;} else { return 1;}")};
							}
				...
				foreach ($this->filters as $filter) {
					...
					switch ($filter['type']) {
						...
						case 'month':
							...
							$form_filters->selectInput($name.'_month',$filter['caption'],false,$value,Stringz::getMonthNames($filter['language']));
							$CFG->o_method_suppress = true;

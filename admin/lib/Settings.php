	...
	public static function getAll() {
		...
		if ($result) {
			foreach ($result as $key => $row) {
				$result[$key]['value'] = Stringz::checkSerialized(html_entity_decode($row['value']));
			}
		}
		return $result;
	}
	...
	public static function getStructured() {
		...
		if ($result) {
			foreach ($result as $row) {
				$name = $row['name'];
				$row['value'] = Stringz::checkSerialized(html_entity_decode($row['value']));
				if (!is_array($row['value'])) {
					$ret[$name] = (stristr($row['value'],'"')) ? htmlentities($row['value']) : $row['value'];
				}
				else {
					$ret[$name] = Stringz::fauxArray($row['value']);
				}
			}
		}
		return $ret;

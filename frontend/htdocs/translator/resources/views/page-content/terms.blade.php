...
<div class="row page-row">
    <div class="justify-content-center page">
        ...
        @endif
            <br><br><br><h4 class="page-title">Terms and Condition</h4>
            ...
            @if($page_content_count<1)
                <form class="modal-form" method="POST" id="orders" action="{{ route('terms.content.store') }}" enctype="multipart/form-data">
            @else
                <form class="modal-form" method="POST" id="orders" action="{{ route('terms.content.edit') }}" enctype="multipart/form-data">
                  ...
                  <div class="row">
                    ...
                    @foreach($language as $languagevlue)
                        ...
                        <?php
                            ...
                            $pges_content = '';
                            if (isset($pge_content->$symbol->terms_condition_heading_key)) {
                        ...
                        <input type="text" class="form-control" id="menu_name" placeholder="Enter Your Page" name="terms_condition_{{$languagevlue->language_symbol}}_heading_key"  value="@if($pges_content){{$pges_content->terms_condition_heading_key}}@endif">
                    ...
                    <input type="hidden" class="form-control"   id="s" placeholder="Enter Your Heading" name="terms_condition_heading_key" required value="terms_condition_heading_key">
                </div>
              </div>
                  ...
                  <div class="row">
                    ...
                    @foreach($language as $languagevlue)
                        ...
                        <?php
                            ...
                            $symbol = (string) $languagevlue->language_symbol;
                            $pges_content = '';
                            if (isset($pge_content->$symbol->terms_condition_content_key)) {
                                // print_r($pge_content->$symbol);
                                $pges_content = $pge_content->$symbol;
                            }
                        ...
                        <label for="name">Content({{$languagevlue->language_symbol}})</label>
                         <textarea name="terms_condition_{{$languagevlue->language_symbol}}_content_key" class="form-control" id="content{{$title}}">@if($pges_content){{$pges_content->terms_condition_content_key}}@endif</textarea>
                        
                  
                        <!-- {{$title++}} -->
                    @endforeach
                        
                    <input type="hidden" class="form-control"   id="s" placeholder="Enter Your Heading" name="terms_condition_content_key" required value="terms_condition_content_key">

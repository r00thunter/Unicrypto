<?php
        ...
        include '../lib/common.php';
			...
			for ($i=0; $i < $currency_count; $i++) { 
				...
				if ($currency_id == 28) {
						if ($currency_id == 28) {
                                $url = "http://$btc_ip/api/receivedTransactions";
                        }elseif ($currency_id == 42) {
                                $url = "http://$ltc_ip/api/receivedTransactions";
                        }elseif ($currency_id == 45) {
                                $url = "http://$eth_ip/api/receivedTransactions";
                        }elseif ($currency_id == 46) {
                                $url = "http://$xrp_ip/api";
                        }elseif ($currency_id == 47) {
                                $url = "http://$xlm_ip/api/receive";
                        }
          ...
          if ($result->status == "success") {
          		for ($j=0; $j < $transaction_count; $j++) { 
          				...
          				// echo "<br><br>";
          				 ...
          				 $transaction_address = $result->body->response->data->txs[$j]->amounts_received[0]->recipient;
          				 $transaction_amount = $result->body->response->data->txs[$j]->amounts_received[0]->amount;

          				API::add('Requests','receivedTransactionIdCheck',array($transaction_id));

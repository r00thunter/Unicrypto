...
<html lang="en">
    <?php include '../lib/common.php';
       ...
       /*  if(!$currency1 || empty($currency1)){
        ...
        API::add('Transactions', 'get', array(1, $page1, 20, $c_currency1, $currency1, 1, $start_date1, $type1, $order_by1));
        $query = API::send();
        $total = $query['Transactions']['get']['results'][0];
        
        API::add('Transactions', 'get', array(false, $page1, 20, $c_currency1, $currency1, 1, $start_date1, $type1, $order_by1));
        ...
        $pagination = Content::pagination('tradehistory.php', $page1, $total, 20, 5, false);
    ...
    <body id="wrapper">
        <?php include "includes/sonance_navbar.php"; ?>
         <?php
        $page_sql=mysqli_query($conn_l, "select id from trans_page where page_status=1 and page_name='trade history'");
        ...
        <header>
            <div class="banner row">
                <div class="container content">
                    <h1><?php echo isset($pgcont['trade_history_heading_key']) ? $pgcont['trade_history_heading_key'] : 'Trade History'; ?></h1>
                     <p class="text-white text-center"><?php echo isset($pgcont['trade_history_sub_heading_key']) ? $pgcont['trade_history_sub_heading_key'] : 'All your Successful completed Buy / Sell transactions are shown here'; ?></p>
                </div>
            </div>
        </header>
                    ...
                    <div class="col-md-12">
                        <form action="" class="form-inline" style="padding: 20px;background: white;margin-top: 20px;">
                            <div class="form-group">
                                <label for="sel1" style="font-size: 12px;"><?php echo isset($pgcont['trade_history_currency_pair_key']) ? $pgcont['trade_history_currency_pair_key'] : 'Currency Pair'; ?> &nbsp;
                                </label>
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="info-table-outer">
                            ...
                            <table id="info-data-table " class="table row-border info-data-table table-hover balance-table table-border" cellspacing="0 " width="100% ">
                                <thead>
                                    <tr>
                                    <th><?php echo isset($pgcont['trade_history_table_head_type_key']) ? $pgcont['trade_history_table_head_type_key'] : 'Type'; ?></th>
                                    <th><?php echo isset($pgcont['trade_history_table_head_date_key']) ? $pgcont['trade_history_table_head_date_key'] : 'Date and Time'; ?></th>
                                    <th><?php echo isset($pgcont['trade_history_table_head_amount_key']) ? $pgcont['trade_history_table_head_amount_key'] : 'Amount'; ?></th>
                                    <th><?php echo isset($pgcont['trade_history_table_head_value_key']) ? $pgcont['trade_history_table_head_value_key'] : 'Value'; ?></th>
                                    <th><?php echo isset($pgcont['trade_history_table_head_price_key']) ? $pgcont['trade_history_table_head_price_key'] : 'Price'; ?></th>
                                    <th><?php echo isset($pgcont['trade_history_table_head_fee_key']) ? $pgcont['trade_history_table_head_fee_key'] : 'Fee'; ?></th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?
                                        if ($transactions) {
                                            foreach ($transactions as $transaction) {
                                                ...
                                                <tr id="transaction_' . $transaction['id'] . '">
                                                    ...
                                                    <td><input type="hidden" class="localdate" value="' . (strtotime($transaction['date'])) . '" />'. ($transaction['date']) .'</td>
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel"><?php echo isset($pgcont['trade_history_currency_pair_modal_head_key']) ? $pgcont['trade_history_currency_pair_modal_head_key'] : 'Trade History'; ?></h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <p><?php echo isset($pgcont['trade_history_currency_pair_modal_content_key']) ? $pgcont['trade_history_currency_pair_modal_content_key'] : 'Select the correct Currency pairs from the dropdown to display their successful transaction details'; ?>.</p>
        <p><?php echo isset($pgcont['trade_history_currency_pair_modal_amount_key']) ? $pgcont['trade_history_currency_pair_modal_amount_key'] : '<b>Amount:</b>The number of cryptocurrencies purchased'; ?>.</p>
        <p><?php echo isset($pgcont['trade_history_currency_pair_modal_value_key']) ? $pgcont['trade_history_currency_pair_modal_value_key'] : '<b>Value:</b> The cost of the cryptocurrency purchased'; ?>. </p>
        <p><?php echo isset($pgcont['trade_history_currency_pair_modal_price_key']) ? $pgcont['trade_history_currency_pair_modal_price_key'] : '<b>Price:</b> The per unit price of the Cryptocurrency purchased (Shown in USD)'; ?></p>
        <p><?php echo isset($pgcont['trade_history_currency_pair_modal_fee_key']) ? $pgcont['trade_history_currency_pair_modal_fee_key'] : '<b>Fee:</b> The fee levied by the Exchange for each transaction'; ?>.</p>
      </div>
    </div>
  </div>
</div>

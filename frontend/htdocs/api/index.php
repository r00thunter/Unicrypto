...
$query = API::send();
API::add('User','setInfo1',array($query['User']['getInfo']['results'][0]));
$query = API::send();
$userid=$query['User']['setInfo1']['results'][0]['id'];
...
// $return['transactions']['market'] = ($c_currency_info['id']) ? $c_currency_info['currency'] : 'ALL';
		...
		// $s_no = 0;
		if ($query['get']['results']['0']) {
		
		foreach ($query as $row) {
			for ($s_no=0; $s_no < count($row['get']['results']['0']); $s_no++) { 

    		if ($row['get']['results']['0'][$s_no]['maker_type'] == 1) {
    			$row['get']['results']['0'][$s_no]['maker_type'] = "BUY";
    		}elseif ($row['get']['results']['0'][$s_no]['maker_type'] == 2) {
    			$row['get']['results']['0'][$s_no]['maker_type'] = "SELL";
    		}
    		
			}
			$return['transactions']  = $row['get']['results']['0'];
	...
	if ($post) {
		if (!$invalid_signature && $api_key1 && $nonce1 > 0) {
			if ($permissions['p_view'] == 'Y') {						
				API::add('User','getBalancesAndInfo_private_api',array($_SESSION['session_id']));				
				API::apiKey($api_key1);				
				...
				// if (empty($query['error'])) {
					$return['balances-and-info'] = $query['User']['getBalancesAndInfo_private_api']['results'][0];
	...
	if ($post) {
		if (!$invalid_signature && !$invalid_currency && !$invalid_c_currency && $api_key1 && $nonce1 > 0) {
			if ($permissions['p_view'] == 'Y') {
				...
				API::add('Orders','get_private_api',array(false,false,false,$c_currency_info['id'],$currency_info['id'],1,false,1,false,false,1,1,'',$_SESSION['session_id']));
				API::add('Orders','get_private_api',array(false,false,false,$c_currency_info['id'],$currency_info['id'],1,false,false,false,1,1,1,'',$_SESSION['session_id']));				
				API::apiKey($api_key1);
				API::apiSignature($api_signature1,$params_json);
				API::apiUpdateNonce();
				$query = API::send($nonce1);
				...
				// if (empty($query['error'])) {
					...
					$return['open-orders']['bid'] = ($query['Orders']['get_private_api']['results'][0]) ? $query['Orders']['get_private_api']['results'][0] : array();
					$return['open-orders']['ask'] = ($query['Orders']['get_private_api']['results'][1]) ? $query['Orders']['get_private_api']['results'][1] : array();
				// }
				// else
				// 	$return['errors'][] = array('message'=>'Invalid authentication.','code'=>$query['error']);
			}
	...
	if ($post) {
		if (!$invalid_signature && !$invalid_currency && !$invalid_c_currency && $api_key1 && $nonce1 > 0) {
			if ($permissions['p_view'] == 'Y') {
				...
				API::add('Transactions','get_private_api',array(false,false,$limit1,$c_currency1,$currency1,1,false,strtolower($type1),false,false,1,'',$_SESSION['session_id']));
				API::apiKey($api_key1);
				API::apiSignature($api_signature1,$params_json);
				API::apiUpdateNonce();
				$query = API::send($nonce1);
				...
				// if (empty($query['error'])) {
					...
					$return['user-transactions']['data'] = ($query['Transactions']['get_private_api']['results'][0]) ? $query['Transactions']['get_private_api']['results'][0] : array();
				// }
				// else
				// 	$return['errors'][] = array('message'=>'Invalid authentication.','code'=>$query['error']);
			}
		...
		if (!$invalid_signature && $api_key1 && $nonce1 > 0) {			
			if ($c_currency_info['id'] > 0 || $invalid_c_currency) {
				if ($permissions['p_view'] == 'Y') {
					...
					API::add('BitcoinAddresses','get_private_api',array(false,$c_currency_info['id'],$c_currency_info['id'],false,false,false,false,1,$_SESSION['session_id']));
					API::apiKey($api_key1);
					API::apiSignature($api_signature1,$params_json);
					API::apiUpdateNonce();
					$query = API::send($nonce1);
					...
					// if (empty($query['error']))
						$return['btc-deposit-address-get'] = ($query['BitcoinAddresses']['get_private_api']['results'][0]) ? $query['BitcoinAddresses']['get_private_api']['results'][0] : array();
					// else
					// 	$return['errors'][] = array('message'=>'Invalid authentication.','code'=>$query['error']);
				}
				else
...
elseif ($endpoint == 'btc-deposit-address/new' || $endpoint == 'crypto-deposit-address/new') {
	if ($post) {
		if ($c_currency_info['id'] > 0 || $invalid_c_currency) {
			if (!$invalid_signature && $api_key1 && $nonce1 > 0) {
				if ($permissions['p_view'] == 'Y') {					
					API::add('BitcoinAddresses','get_private_api',array(false,$c_currency_info['id'],false,1,1,'','','',$_SESSION['session_id']));
					API::apiKey($api_key1);
					API::apiSignature($api_signature1,$params_json);
					$bitcoin_addresses = $query['BitcoinAddresses']['get_private_api']['results'][0];
					$query = API::send($nonce1);										
					...
					if (empty($error)) {
						API::add('BitcoinAddresses','getNew_private_api',array($c_currency_info['id'],1,$_SESSION['session_id']));
						API::apiKey($api_key1);
						API::apiSignature($api_signature1,$params_json);
						API::apiUpdateNonce();
						$query = API::send($nonce1);
						...
						// if (empty($query['error']))
							$return['crypto-deposit-address-new']['address'] = $query['BitcoinAddresses']['getNew_private_api']['results'][0];
						// else
						// 	$return['errors'][] = array('message'=>'Invalid authentication.','code'=>$query['error']);
					}
				}
	...
	if ($post) {
		if (!$invalid_signature && !$invalid_currency && $api_key1 && $nonce1 > 0) {
			if ($permissions['p_view'] == 'Y') {
				...
				if (empty($error)) {
					API::add('Requests','get_private_api',array(false,false,$limit1,false,$currency_info['id'],$status1,1,'',$_SESSION['session_id']));
					API::apiKey($api_key1);
					API::apiSignature($api_signature1,$params_json);
					API::apiUpdateNonce();
					$query = API::send($nonce1);
					...
					// if (empty($query['error']))
						$return['deposits'] = ($query['Requests']['get_private_api']['results'][0]) ? $query['Requests']['get_private_api']['results'][0] : array();
					// else
					// 	$return['errors'][] = array('message'=>'Invalid authentication.','code'=>$query['error']);
				}
			}
	...
	if ($post) {		
		if (!$invalid_signature && !$invalid_currency && $api_key1 && $nonce1 > 0) {
			if ($permissions['p_view'] == 'Y') {
				...
				API::add('Requests','get_private_api',array(false,false,$limit1,1,$currency_info['id'],$status1,1,'',$_SESSION['session_id']));								
				API::apiKey($api_key1);
				API::apiSignature($api_signature1,$params_json);
				API::apiUpdateNonce();
				$query = API::send($nonce1);
				...
				// if (empty($query['error']))
					$return['withdrawals'] = ($query['Requests']['get_private_api']['results'][0]) ? $query['Requests']['get_private_api']['results'][0] : array();
				// else
				// 	$return['errors'][] = array('message'=>'Invalid authentication.','code'=>$query['error']);
			}
			else
			...
			if ($permissions['p_orders'] == 'Y') {
				...
				if (is_array($orders)) {
					...
					foreach ($orders as $order) {
						...
						API::add('Orders','executeOrder_private_api',array(($order['side'] == 'buy'),$order['limit_price'],$order['amount'],$order['market'],$order['currency'],false,($order['type'] == 'market'),false,false,false,$order['stop_price'],false,1,'',$_SESSION['session_id']));						
						API::apiKey($api_key1);
						API::apiSignature($api_signature1,$params_json);
						...
						$query = API::send($nonce1);					
						$result = $query['Orders']['executeOrder_private_api']['results'][0];
			...
			if ($permissions['p_orders'] == 'Y') {
				...
				if (is_array($orders)) {
					...
					foreach ($orders as $order) {
						...
						API::add('Orders','executeOrder_private_api',array(false,$order['limit_price'],$order['amount'],false,false,false,($order['type'] == 'market'),$order['id'],false,false,$order['stop_price'],false,1,'',$_SESSION['session_id']));						
						API::apiKey($api_key1);
						API::apiSignature($api_signature1,$params_json);
				...
				if (empty($_POST['all'])) {
					...
					if (is_array($orders)) {
						...
						foreach ($orders as $order) {
							...
							$order['id'] = (!empty($order['id'])) ? preg_replace("/[^0-9]/", "",$order['id']) : false;
							API::add('Orders','delete_private_api',array(false,$order['id'],$_SESSION['session_id']));
							...
							$query = API::send($nonce1);
							$result = $query['Orders']['delete_private_api']['results'][0];
			...
			if ($permissions['p_view'] == 'Y') {
				...
				if (is_array($orders)) {
					...
					foreach ($orders as $order) {
						...
						API::add('Orders','getStatus_private_api',array($order['id'],1,$_SESSION['session_id']));
						API::apiKey($api_key1);
						API::apiSignature($api_signature1,$params_json);
						...
						$query = API::send($nonce1);
						$result = $query['Orders']['getStatus_private_api']['results'][0];
			...
			if ($permissions['p_withdraw'] == 'Y') {
				...
				elseif ($currency_info['is_crypto'] != 'Y' && !$account1)
					...
					if ($currency_info['is_crypto'] == 'Y') {
						API::add('User','getAvailable_Private_API',array($_SESSION['session_id']));
						API::add('BitcoinAddresses','validateAddress',array($address1));						
						API::add('Wallets','getWallet',array($currency_info['id']));
						API::apiKey($api_key1);
						API::apiSignature($api_signature1,$params_json);
						$query = API::send($nonce1);
						$user_available = $query['User']['getAvailable_Private_API']['results'][0];
						...
						API::add('BankAccounts','getRecord_private_api',array(false,$account1,$_SESSION['session_id']));
						API::add('BankAccounts','get_private_api',array($currency_info['id'],$_SESSION['session_id']));
						API::add('User','getAvailable_Private_API',array($_SESSION['session_id']));
						API::apiKey($api_key1);
						API::apiSignature($api_signature1,$params_json);
						$query = API::send($nonce1);
						$bank_account = $query['BankAccounts']['getRecord_private_api']['results'][0];
						$bank_accounts = $query['BankAccounts']['get_private_api']['results'][0];
						$user_available = $query['User']['getAvailable_Private_API']['results'][0];
					...
					if (!$error) {
						API::add('Requests','insert_private_api',array($currency_info['id'],$amount1,$address1,$account1,$_SESSION['session_id']));						
						API::apiKey($api_key1);
						API::apiSignature($api_signature1,$params_json);
						API::apiUpdateNonce();
						$query = API::send($nonce1);
	
						$return['withdraw'] = ($query['Requests']['insert_private_api']['results'][0]) ? $query['Requests']['insert_private_api']['results'][0] : array();
					}
				}
			}
			else

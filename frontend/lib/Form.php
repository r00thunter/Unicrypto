...
class Form {
	...
	function textInput($name, $caption = false, $required = false, $value = false, $id = false, $db_field_type = false, $class = false, $jscript = false, $style = false,$is_manual_array=false,$is_unique=false,$unique_error=false,$default_text=false,$delete_whitespace=false,$static=false,$j=false,$grid_input=false) {
		...
		$HTML =  "<div class=\"label_extend\"></div><label for=\"{$this->name}_{$id}{$id_j}\">$caption  $req_img </label><input type=\"text\" name=\"{$this->name}[$name]{$mult}\" value=\"$value\" id=\"{$this->name}_{$id}{$id_j}\" $class ".$jscript." $style $onblur $onclick />";
		if (!$static) {
			$this->HTML[] = $HTML;
		}
		else {
	...
	function passwordInput($name, $caption = false, $required = false, $value = false, $id = false, $class = false, $jscript = false, $style = false,$compare_with=false) {
		...
		$this->HTML[] = $req_img."<div class=\"label_extend\"></div><label for=\"{$this->name}_{$id}\">$caption </label><div class=\"input-group\"><input type=\"password\" name=\"{$this->name}[$name]\" value=\"$value\" id=\"{$this->name}_{$id}\" $class ".$jscript." style=\"border-right: 0px;\"/>
                           	<span class=\"input-group-addon\" style=\"border: 1px solid #ced4da;border-left: 0px;padding: 7px 10px 0px 10px;\"><i class=\"fas fa-eye-slash\" onclick=\"showpassword{$this->name}_{$id}(this);\"></i></span></div>";
	...
	function checkBox($name,$caption=false,$required=false,$id=false,$class=false,$jscript=false,$style=false,$checked=false,$label_class=false,$static=false,$j=false,$grid_input=false) {
		...
		$HTML = $req_img."<div class=\"label_extend\"></div><input type=\"checkbox\" class=\"checkbox\" name=\"{$this->name}[$name]{$mult}\" value=\"Y\" id=\"{$this->name}_{$id}{$j}\" ".$jscript." $style $checked /><label $label_class for=\"{$this->name}_{$id}{$j}\">$caption </label> <input type=\"hidden\" name=\"checkboxes[]\" value=\"$name\" />";
	...
	function selectInput($name,$caption=false,$required=false,$value=false,$options_array=false,$subtable=false,$subtable_fields=false,$subtable_f_id=false,$id=false,$class=false,$jscript=false,$style=false,$f_id_field=false,$default_text=false,$depends_on=false,$function_to_elements=false,$static=false,$j=false,$grid_input=false,$first_is_default=false,$level=false) {
		...
		$mult = ($static) ? '['.$j.']'.(($grid_input) ? '['.$grid_input.']' : '') : '';
		$HTML = "<div class=\"label_extend\"></div><label for=\"{$this->name}_{$id}{$j}\">$caption $req_img </label><select name=\"{$this->name}[$name]{$mult}\" id=\"{$this->name}_{$id}{$j}\" $class $style>";
		if (!$first_is_default)
		...
		if ($this->errors) {
			if (!$this->errors_to_function) {
				...
				foreach ($this->errors as $name => $error) {
					echo '<div><div class="error_icon"></div>'.ucfirst(str_ireplace('[field]',$name,$error)).'</div>';
				}
		...
		if ($this->messages) {
			if (!$this->errors_to_function) {
				...
				foreach ($this->messages as $name => $message) {
					echo '<div><div class="warning_icon"></div>'.ucfirst(str_ireplace('[field]',$name,$message)).'</div>';
				}
				echo '</div><div class="clear"></div>';
			}
			else {
		...
		if (is_array($this->HTML)) {
			...
			foreach ($this->HTML as $method_id => $elem) {
				...
				elseif (stristr($elem,'<group>')) {
					$in_group = true;
					echo str_replace('<group>','','<div class="'.$alt.'">'.$elem);
					continue;
				}
				elseif (stristr($elem,'</group>')) {
					$in_group = false;
				...
				elseif ($elem == '<captcha>') {
					if (!empty($CFG->google_recaptch_api_key) && !empty($CFG->google_recaptch_api_secret)) {
						echo '<div class="'.$alt.'"><div class="g-recaptcha" data-sitekey="'.$CFG->google_recaptch_api_key.'"></div></div>';
					}
					else {
						echo "
						<div class=\"$alt\">
				...
				elseif (stristr($elem,'<new_editor>')) {
					echo "<div class=\"editor_li $alt\">".str_replace('<new_editor>','',$elem).'</div>';
					continue;
				}
				...
				$alt="form-group";
				if (!$this->group && !stristr($elem,'<fieldset') && $elem != '</fieldset>' && !stristr($elem,'class="hidden_input"')) echo "<div class=\"$alt\">";
				echo $elem;
				if (!$this->group && !stristr($elem,'<fieldset') && $elem != '</fieldset>' && !stristr($elem,'class="hidden_input"')) echo '</div>';
				if ($this->group) echo '&nbsp;&nbsp;&nbsp;'; 
			}
		}
		...
		if ($this->info && $this->verify_fields) {
			foreach ($this->info as $name => $value) {
				if (array_key_exists($name,$this->verify_fields)) {
					...
					if (strstr($this->verify_fields[$name],'phone')) {

						/* if(strlen($value)>10)
						$this->errors[$name] = 'Phone numbers only ten digits';  */

						/* if (!preg_match('/^[0-9]*$/',$value)) 
							$this->errors[$name] = 'Please enter valid phone number'; */

							API::add('User','phoneExists',array($value));
								$query = API::send();
							if(!empty($query['User']['phoneExists']['results'][0]))
							$this->errors[$name] = 'Phone number already exists';	
					}
					/**
                     * check email in database before register user
                     */
                    if (strstr($this->verify_fields[$name],'email')) {

                        API::add('User','userExists',array($value));
                        $query = API::send();
                        if(!empty($query['User']['userExists']['results'][0]))
                        $this->errors[$name] = 'Email already exists';  
                    }
